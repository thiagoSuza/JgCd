...
public class GetData4 : MonoBehaviour
{
    ...
    IEnumerator Startt()
    {
        using (UnityWebRequest www = UnityWebRequest.Get(DATABASE_URL + "4.json"))
        {
    ...
    IEnumerator GetDataR()
    {
        ...
        {
            RestClient.Get<Data>(DATABASE_URL + "4/" + st[i] + ".json").Then(response =>
            {
    ...
    IEnumerator HighScore(int x)
    {
        ...
        RestClient.Get<Data>(DATABASE_URL + "4/" + st[x] + ".json").Then(response =>
        {
