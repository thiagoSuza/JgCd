...
public class GetData3 : MonoBehaviour
{
    ...
    IEnumerator Startt()
    {
        using (UnityWebRequest www = UnityWebRequest.Get(DATABASE_URL + "3.json"))
        {
    ...
    IEnumerator GetDataR()
    {
        ...
        {
            RestClient.Get<Data>(DATABASE_URL + "3/" + st[i] + ".json").Then(response =>
            {
    ...
    IEnumerator HighScore(int x)
    {
        ...
        RestClient.Get<Data>(DATABASE_URL + "3/" + st[x] + ".json").Then(response =>
        {
