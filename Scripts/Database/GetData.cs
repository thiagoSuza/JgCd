...
public class GetData : MonoBehaviour
{
    ...
    IEnumerator Startt()
    {
        using (UnityWebRequest www = UnityWebRequest.Get(DATABASE_URL + "1.json"))
        {
    ...
    IEnumerator GetDataR()
    {
        ...
        {
            RestClient.Get<Data>(DATABASE_URL + "1/" + st[i] + ".json").Then(response =>
            {
    ...
    IEnumerator HighScore(int x)
    {
            ...
            RestClient.Get<Data>(DATABASE_URL + "1/" + st[x] + ".json").Then(response =>
            {
